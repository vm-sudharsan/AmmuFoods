        ...
        <div className="min-h-screen flex flex-col bg-brand-beige">
            ...
            <div className="py-20 bg-white">
                <div className="max-w-7xl mx-auto px-4 text-center">
                    <h2 className="text-4xl font-serif font-bold text-brand-brown mb-6">Authentic Taste of Tradition</h2>
                    <p className="text-gray-600 text-lg max-w-2xl mx-auto leading-relaxed">
                        Experience the true flavors of homemade delicacies, prepared with the finest ingredients and love.
                        From our signature sweets to refreshing drinks, every bite is a celebration.
                    </p>
                </div>
            </div>

            {/* Featured Delicacies */}
            <div className="py-16 bg-brand-beige/30">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <h2 className="text-4xl font-serif font-bold text-center text-brand-brown mb-12">Featured Delicacies</h2>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {products.length > 0 ? (
                            products.map(product => (
                                <ProductCard
                                    key={product._id}
                                    image={product.imageUrl || ellaneerPayasam} // Fallback to a default image if URL is missing/broken
                                    title={product.name}
                                    weight={product.unit}
                                    price={product.price}
